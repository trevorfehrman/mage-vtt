...
import { containmentCap, toGnosisRank, waitingOn, type CastStatus } from "#/domain/cast"
...
export function CastCard({
  ...
  // The Paradox pool as the engage beat froze it (a computed default,
  // ADR-0015) — the same leaf the server rolls with, never a reimplementation.
  const poolInputs =
    cast.gnosis !== undefined
      ? {
          gnosis: toGnosisRank(cast.gnosis),
          usesMagicalTool: cast.usesMagicalTool,
          sleeperWitnesses: cast.sleeperWitnesses ?? false,
...
/** A plain capped number input (issue #43: previews come in a later slice). */
function NumberInput({
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <button
        onClick={() => onChange(Math.min(clamped + 1, cap))}
        disabled={disabled || clamped >= cap}
        className="mv-btn grid size-6 place-items-center rounded-[3px] text-[13px] leading-none disabled:opacity-40"
      >
        +
      </button>
