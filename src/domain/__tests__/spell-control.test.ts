...
import { canMaintainSpell, relinquishSpell } from "../spell-control"

describe("Spell Control", () => {
  it("can maintain spells up to gnosis limit without penalty", () => {
    // Gnosis 2 = 2 free maintained spells
    const ok = canMaintainSpell({ gnosis: 2, currentlyMaintained: 1 })
    expect(ok.canMaintain).toBe(true)
    expect(ok.penalty).toBe(0)

    // At limit
    const atLimit = canMaintainSpell({ gnosis: 2, currentlyMaintained: 2 })
    ...
    const over = canMaintainSpell({ gnosis: 1, currentlyMaintained: 3 })
